...
- **Dreamer: self-eating guard — records quoting dreamer output are provenance-excluded from fact extraction** (`mnemo-dream.py`, `_is_dreamer_derived` + `DREAMER_OUTPUT_SIGNATURES`; tests in `tests/test_dream_provenance.py`). Problem this fixes: the dreamer's own fan-out artifacts re-entered as evidence through OTHER agents' lanes — an agent processes a `dream-contradictions-*` ping, its memory record quotes the subject line, the next harvest reads that record under the agent's provenance, and extraction manufactures a "fact" out of the dreamer's own output (2026-08-14 specimen: Rocky's "Processed Disco Bus message #2401: dream-contradictions for Opie's role" extracted role="dream-contradictions" and flagged it against the verified fact it descends from — a compounding loop, since each dream about a topic manufactures tomorrow's evidence on the same topic). The lane-level harvest exclusion cannot catch these; this is a deterministic content-signature check applied before any semantic filtering, extraction-only (synthesis still sees the records), never-silent (excluded records are logged by session id). Paraphrase handling is explicitly out of scope — that is the narrative-tier mechanism, tracked separately.
...
- **Fix: CI has been red since 2026-08-07 — three leftovers from the two changes above.** Nine consecutive failed runs, three of the four jobs, all traceable to the one commit. (1) `test (3.11)`: the UTF-8 sweep put `encoding="utf-8"` inside an f-string already delimited by double quotes (`agentb/cli.py:1678`) — legal only from Python 3.12 (PEP 701), a `SyntaxError` on 3.11, which `pyproject` declares supported and CI matrix-tests. The whole test module failed at *collection*, so 667 unrelated tests never ran; `test (3.12)` stayed green, which is why this read as noise. Fixed by hoisting the manifest read to its own statement, matching the passport line above it. (2) `docker-boot`: the `Dockerfile` still ran `COPY sparks_bus/ sparks_bus/` for a directory that no longer exists — build failed at checksum. (3) `wheel-smoke`: the smoke test still asserted `sparks_bus` was an importable package shipping `schema.sql`. The package-data assertion was removed rather than emptied — a loop over an empty list reports success without checking anything (`doctrine-clean-is-not-a-check`). Verified before commit: full suite `668 passed` under a real 3.11 interpreter, plus `wheel-smoke-test.sh`, `docker build`, and the `import agentb.server` boot check, each run bare for its own exit code.

- **Fix: literal `~` in YAML path fields is now expanded at the config-parse boundary** (`data_dir`, per-agent `data_dir`, `storage.path`). Problem this fixes: YAML never expands `~`, and `Path("~/x")` is relative, so a configured `data_dir: ~/.config/agentb/...` was silently created under the process CWD as a directory literally named `~` — any test run or `import agentb.server` (module-level `create_app()`) planted one wherever it was run from. Four such stray trees accumulated on IGOR between 2026-03 and 2026-08. Regression tests in `tests/test_config_paths.py`; empty path values stay empty rather than expanding to `"."`. Same fix for `AGENTB_CONFIG` (a tilde there — e.g. from a systemd `Environment=` line — used to make the config silently unreadable and fall through to an all-defaults config; health.py already expanded, so the doctor and the loader could disagree).
