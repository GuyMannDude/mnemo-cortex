...
PY
  ...
  # Create it 0600 BEFORE writing: write-then-chmod leaves a window where
  # the key is world-readable on a multi-user host.
...
ExecStart=$PY_BIN -m uvicorn agentb.server:create_app --factory --host $SVC_HOST --port $SVC_PORT --log-level info
Restart=on-failure
RestartSec=5
