...
**By sound, when the spelling is unseen (v4.22).** A misspelled proper name matches no word at all — "Elenore" when the memory says "Eleanor", "Fershow" for "Fershaw". Each memory therefore also stores the consonant-skeleton keys of its words in a second column of the same index, and a query word of four or more letters that *no* memory contains adds its key to the match. A correctly spelled name never picks up its sound-alikes, an identifier never takes part, and the key is built from zero (Soundex folds too much: "guy"/"gay"). Off switch: `ranking.phonetic_enabled: false`.
